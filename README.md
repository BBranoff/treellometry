Allometric equations for mangrove biomass estimation: Evaluating
multiple variables in mixed effects models using a global data
compilation
================
...
distributions have relatively little variance compared to short and flat
distributions. These visualizations help understand how the data are
distributed among the groups and how the individual groups fit into the
overall ‘population’, which is an underlying basis for testing in mixed
effects models.
...
RMSE from those values. This correction takes the form of:
$$estimate = exp(\hat{y}+\frac{\sigma^2}{2})$$ where $\hat{y}$ is the
...
metrics. Remember, however, that these rankings are somewhat arbitrary
and based primarily upon our ability to logistically sort a large number
of models. Different rankings could be applied for different objectives
(prioritize RMSE, for example). Still, this gives us a general idea of
which models are performing ‘better’ than others. We are left with
around 100 models, which is still a considerable number.
...
compare_perfomance function from the performance package. This function
...
such a way so as to replicate the output of compare_performance(). We
then scale the metric values so that they are comparable across all of
the families. Although doing this makes it slightly more difficult to
...
In the plots, the most complex model, with random slopes and intercepts
on species and site, seems to perform ‘better’. But this is debatable
depending on which metrics are prioritized. If BIC (which penalized
complexity) is prioritized, a simpler model with only random effects on
site, may be optimal. Either way, its clear that in general, for the
same set of variable, mixed effects models out-perform the fixed effects
versions. This is the primary intention of mixed-effects modelling when
hierarchical data is suspected. By partitioning the variance into groups
and fitting coefficients separately, mixed-effects models are better
able to capture the global and group specific trends.
...
graphic, we have aggregated all models for simplicity, so we cant see
what var1, var2, var3 etc. are, but variable specific graphics are
...
effects dominate, accounting for the vast majority of variance (Fig 4a).
With fixed effects removed (Fig. 4b), its easier to compare the random
...
![](README_files/figure-gfm/varpart2-1.png)<!-- -->
...
![](README_files/figure-gfm/varpart2-2.png)<!-- -->
...
alt="Fig. 5 Random effects for the three univariate models. Deviations from the zero line indicate a potentially significant random effect. Species exhibited non-significant random effects, although there were only three species in this study and all were from the Atlantic geo-region. Location, however, does demonstrate some significant random effects, especially for height. This suggests that height as a predictor of biomass may be location specific." />
<figcaption aria-hidden="true">Fig. 5 Random effects for the three
...
To test model overfitting and general applicability to ‘new’ data, we
also perform a cross validation. Some modelling processes incorporate a
data splitting into ‘training’ and ‘testing’ subsets. The ‘training’
subset is typically around 70% of the original dataset and is used to
fit the models. These models are then ‘tested’ on the other 30% of the
data to see how well they perform on unseen observations. This allows us
to examine how well the models perform on new data. Cross validation
(CV) takes this a bit further by ‘rotating’ the training and testing
subsets, which helps avoid results that may only be a result of a
‘lucky’ (or unlucky) training set.

Here, we incorporate 10 ‘folds’ of the data, or 10 rotations of the
training-testing subsets and get RMSE values for each of the 10
predictions on the testing subsets. When the RMSE values from the 10
folds are similar, this indicates the models and their coefficients are
consistent at predicting new data. When they deviate, however, this
indicates that predictions are unstable and may not predict well on new
data.

The below figure demonstrates the CV results from our models. Generally,
the results indicate that for models incorporating fixed effects only or
random intercepts, predictions are stable and may be applicable to new
data. For models incorporating random slopes, however, CV indicates
