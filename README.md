...
This is a placeholder for the Treellometry R package. Future project
repositories will be placed here.

For the Ecoinformatics paper entitled: “Local vs global allometric
equations: Evaluating multiple variables in mixed effects models for
biomass estimation using a global mangrove data compilation”, navigate
to the [ecoinf
branch](https://github.com/BBranoff/treellometry/tree/ecoinf)

## Description

This branch of the repository describes generalized algorithms for
assessing tree allometry. For now, this includes:

- Comparing fixed and mixed effects models in predicting tree above
  ground biomass from standard morphological measurements.
...
can be done by installing the package in R as shown below.

``` r
##  to install this repository directly, the installation function (remotes) first need to be installed
install.packages("remotes")
##  then install from github
remotes::install_github("BBranoff/treellometry")
library(treellometry)
library(dplyr)
```

With the library successfully loaded, the following methods will
demonstrate a workflow to fit and compare a robust set of models to
predict tree above ground biomass. The models are performed on a dataset
of harvested mangrove trees in which aboveground biomass was taken along
with various morphological measurements (height, diameter, etc.)

The first steps are to create the composite variables (product of
variables). These are meant to roughly represent tree form (volume) and
are commonly used in allometric modelling because two allometric
covariables are likely to be correlated, which destabilizes estimates in
multivariate models. Creating a composite variable eliminates this
issue, although it does introduce interpretation considerations, as the
product of variables isn’t always directly relatable to tree form. Here,
we also set the response variable, which usually aboveground biomass, as
well as the various predictor variables. We also establish the grouping
variables, species and location. These will be used to model random
effects, which aim to investigate whether species and location are
...
the analysis, which includes fitting models of biomass from various
combinations of predictor variables and fixed and mixed effects, as well
as retrieving model coefficients and fitness metrics used in model
evaluation. Rather than leaving all of that in the black box of the
function, here we break down the internal steps of
...
reason, coefficients should generally be reported on models from the
non-scaled, but still log transformed data, while performance metric
