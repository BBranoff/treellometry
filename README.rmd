...
```{r varpart-demo,fig.height=10, message=FALSE,warning=FALSE,cap="Fig. 5 Variance decomposition of the models."}
library(lme4)

```

```{r varpart,fig.height=10, message=FALSE,warning=FALSE,cap="Fig. 5-6 Partitioning the variance in aboveground biomass to fixed effects, random effects, and residuals; means and standard errors for each component within a model family. Total explained variance is at least 90% for all model families, and fixed effects dominate in all models, accounting for the vast majority of variance (Fig 5).  With fixed effects removed (Fig. 6), its easier to compare the random effects, which account for no more than roughly 6% of the variance. This corroborates the view that species and location level information provide relatively little to the variability in allometric relationships."}
library(insight)
...
# ```{r varpart2, eval=FALSE, echo=FALSE, message=FALSE,warning=FALSE"}
# ##  random effects
